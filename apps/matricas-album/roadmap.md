...
### 2026-06-01 — Gold-standard `726106121` subtree ingested; REFACTOR-001 plan created

The `3. Rendszerstruktúra és alapfogalmak` subtree (`726106121`) was ingested as the product gold standard (wiki: `docs/llm-wiki/wiki/summaries/2026-06-01-rendszerstruktura-gold-standard.md`; raw snapshot: `docs/llm-wiki/raw/confluence/rendszerstruktura-es-alapfogalmak/2026-06-01/`, dated for delta-tracking). It promotes the full `Tanterv → Modul → Témakör → Blokk → Tevékenység` hierarchy to **real, reference-composed, versioned entities**, plus a closed `Tevékenységtípus` taxonomy and a mode-chooser / 3-pane builder creation UX — i.e. it elevates **Stream 5** ("Future activity-domain generalization") from deferred preparation to planned work, still under the pilot-safe / compatibility-first / teacher-reviewed guardrails. Detailed phased plan: **`docs/llm-wiki/wiki/plans/REFACTOR-001-gold-standard-rendszerstruktura.md`** (lives in the wiki per the "plans live in the wiki" rule, AGENTS.md §2). Live contradictions C1–C5 (the spec drops `Tanulási egység` contra ADR002; reframes `matrica` as a reward element; root-vs-detail hierarchy mismatch; PNG-only `Feladat`; platform-vs-product scope) are **flagged, not resolved**, and gate the schema phases behind REFACTOR-001 Task 2.1 (a product decision memo). The **creation-UX simplification** (REFACTOR-001 Phase 1, Stream 3) is **not** gated and ships first.
