...
| --- | --- | --- |
| `Tevékenység (matrica)` | `CreateStickerPayload` -> `StickerResource` / `StickerVersion` | Existing sticker API. The dominant `Tevékenységtípus` (`ActivityTypeKey`) and the activity-planning metadata (`Subject`, `GradeLevel`, `EstimatedMinutes`, `Modality`, `GroupSize`, `ContextMode`, `CompetenciesJson`, `NatReferencesJson`) are now real persisted columns; the create drawer sends them as a structured `metadata` object and the server also promotes any legacy note-lines via `ActivityMetadataNotes.Parse`. Only the hierarchy context (`Tanterv → Blokk`) and free-text local context remain teacher-step note-lines until they become Block/Topic entities (Phases 4–6). The activity detail also exposes read-only `relations` (`reusedIn` template usage + `derivedFrom` version lineage). |
