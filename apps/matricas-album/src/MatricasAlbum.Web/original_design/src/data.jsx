...
const STATE_ORDER = ['tervezett','aktiv','bekuldve','varakozik','javitas','elkeszult','reflektalt'];

const TEAMS = [
  { id: 'arnyek',  name: 'Árnyékkommandó',   members: ['Dóri','Marci','Hanna','Zétény'], focus: 'Növényzet és árnyék', color: '#7872d4' },
  { id: 'kovek',   name: 'Kőkutatók',         members: ['Bence','Petra','Réka','Tomi'],   focus: 'Burkolatok hatása', color: '#ef7c5b' },
  { id: 'felho',   name: 'Felhőfigyelők',     members: ['Vince','Léna','Boti','Adél'],    focus: 'Mikroklíma és időjárás', color: '#5cb6a3' },
  { id: 'viz',     name: 'Vízkereső expedíció', members: ['Eszter','Csongor','Ági','Misi'], focus: 'Víz és párolgás', color: '#e5b653' },
];
...
const INITIAL_STICKERS = [
  // Week 1
  {
    ...
    studentChoice: 'Választhattok: árnyékos és napos terület összehasonlítása, burkolatok összehasonlítása, növényzet szerepe vagy emberi használat megfigyelése.',
    ...
    bPlan: 'Esős időben használjatok korábbi iskolai fotókat vagy az iskola térképes nézetét. A megfigyelést végezhetitek ablakból, csak a hipotézis nem maradhat el.',
    lowResource: 'Telefon nélküli csapatok rajzos vázlatot készítenek a két helyszínről, mellé szöveges leírást a felület és az árnyék állapotáról.',
  },
  // Week 2
  {
    id: 's2',
    ...
    phase: 'kepzelet',
    state: 'elkeszult',
    short: 'Perspektívák ütköztetése: kisdiák, sportoló, tanár, takarító, kerékpáros nézőpontja.',
    studentInstruction: 'Húzzatok egy szerepkártyát (alsós gyerek, focista, tanár, takarító, kerékpáros). Az ő nézőpontjából írjátok le, milyen az élhető udvar, és milyen az, ami zavar a hőségben.',
    teacherSteps: [
    ...
    bPlan: 'Ha a vita túl gyorsan eljut konszenzusig, dobj be egy ellentmondó forrást (pl. nyári hőségriasztás cikk részlete).',
  ...
  // Week 3
  {
    ...
    studentInstruction: 'Mérjetek 3 különböző helyszínen hőmérsékletet (napos burkolat, árnyékos fűfelület, és egy harmadik általatok választott pont). Készítsetek mérési táblázatot napszakonként.',
    teacherSteps: [
      ...
      'Az adatfelvitel után közös elemzés: melyik tényező hatott legjobban?',
    ],
    ...
    bPlan: 'Ha kevés a hőmérő, csapatok rotálják őket 15 perces időkeretekben, és a köztes időben a megfigyelési jegyzet készül.',
    lowResource: 'Digitális hőmérő helyett árnyék–nap összehasonlító megfigyelési lap: vizuális becslés és tapintás-alapú jegyzet.',
  },
  // Week 4
  {
    id: 's4',
    ...
    studentInstruction: 'Készítsetek 3 perces bemutatót: mit javasoltok az iskola hőtűrésének javítására, és melyik mérési adat támasztja alá?',
    teacherSteps: [
...
const INITIAL_EVIDENCE = [
  {
    ...
    description: 'A déli oldalon álló műfüves felületnél a járda is forró volt. A jegyzetben azt írtuk, hogy a fűz fa körüli rész lényegesen hűvösebbnek tűnt.',
    status: 'elkeszult',
    teacherFeedback: 'Jó megfigyelés. A következő körben próbáljátok pontosabban leírni a mérés időpontját.',
  },
  {
    ...
    description: 'A kisebbeknek a délutáni napsütésben nincs hová leülniük az udvaron. Az érvtérkép a játszótér árnyékhiányára épült.',
    status: 'elkeszult',
    teacherFeedback: 'Erős nézőpont. A példák konkrétak. Köszönöm a jól dokumentált érvtérképet.',
  },
  {
    ...
    description: 'A napos műfüves részen 34°C-ot mértünk, az árnyékos fás részen 28°C-ot. Szerintünk a burkolat és az árnyék együtt számít.',
  ...
  },
  {
    ...
    description: 'Aszfalt 36°C, kavics 32°C. Csak két mérést tudtunk venni, mert egy hőmérő hibásan mutatott.',
  ...
  },
  {
    ...
    description: 'Két helyszínen mértünk, az adatok közel azonosak. A módszer leírása nem pontos: nem jegyeztük le a mérés idejét.',
...
/* Lannert / creative learning quality dimensions */
const QUALITY_DIMS = [
  ...
  { id: 'lowres',   label: 'Low-resource megvalósíthatóság', score: 55, state: 'warn' },
...
function StoreProvider({ children }) {
  const [role, setRole] = useState('teacher'); // teacher | student | closure
  const [page, setPage] = useState('home'); // teacher pages
  const [studentPage, setStudentPage] = useState('current');
  const [stickers, setStickers] = useState(INITIAL_STICKERS);
  const [evidence, setEvidence] = useState(INITIAL_EVIDENCE);
  const [microStickerProposed, setMicroProposed] = useState(true);
  const [microStickerAccepted, setMicroAccepted] = useState(false);
  const [activeStickerId, setActiveStickerId] = useState(null);
  const [activeEvidenceId, setActiveEvidenceId] = useState(null);
  const [toast, setToast] = useState(null);
  const [wizardOpen, setWizardOpen] = useState(false);
  const [printOpen, setPrintOpen] = useState(null); // 'weekly' | 'closure' | null
  const [teamId] = useState('arnyek'); // student's team
  ...
  const acceptMicroSticker = () => {
    setMicroAccepted(true);
    showToast('„Mérési gyorstalpaló” beillesztve a 2. hét végére.', 'auto_awesome');
  };
  ...
  const value = {
    ...
    updateSticker, updateEvidence,
    microStickerProposed, setMicroProposed,
    microStickerAccepted, setMicroAccepted, acceptMicroSticker,
    ...
    teamId,
    evidenceFlowOpen, setEvidenceFlowOpen,
    ALBUM: ALBUM_META,
    TEAMS, PHASES, STATES, QUALITY_DIMS,
  };
  return <StoreContext.Provider value={value}>{children}</StoreContext.Provider>;
}

Object.assign(window, {
  PHASES, STATES, STATE_ORDER, TEAMS, ALBUM_META, QUALITY_DIMS,
  INITIAL_STICKERS, INITIAL_EVIDENCE,
