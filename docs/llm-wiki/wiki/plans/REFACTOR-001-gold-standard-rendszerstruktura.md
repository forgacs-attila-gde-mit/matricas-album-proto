...
- **Task 4.1** — ✅ **Done 2026-06-01** — `Block` + `BlockVersion` entities (mirror album-template versioning: draft/publish, ≤1 draft via partial unique index; v1 starts editable) — `Domain/Block.cs`, `Migrations/20260601150000_AddBlocks.cs`, DbContext, endpoints behind `Features:Hierarchy:Block` (on in Development). `BlockFlowTypes`/`BlockGroupings` defaulted + `BlockActivityRoles` closed. Verified live.
- **Task 4.2** — ✅ **Done 2026-06-01** — `ActivityBlockRelation` (reference, not embed; FK restrict to `StickerVersion`) with `Role` (closed set), `SortOrder`, `flowType`/`grouping` on the block version — endpoints `POST/DELETE /api/blocks/{id}/activities`, two-pass `/reorder`. All four behaviours verified live (references-not-copies, roles enforced→400, reorder no-collision, same activity in two blocks). `rules` field deferred (not in the collected data yet).
- **Task 4.3** — ✅ **Done 2026-06-01** — Block builder UX (`BlockBuilderComponent`) composes `CreationShellComponent`: left = searchable activity library (add-by-reference with a role), center = CDK drag-&-drop flow (reorder + role + remove), right = live ordered preview. Route `/teacher/blocks` + flag-gated „Blokkműhely" nav (`core/tokens/features.ts`). Added `PATCH /api/blocks/{id}/activities/{relationId}` (role change) to back the role selector. 26/26 ng-test (`builder_dragdrop`, `assign_role`, `preview_order` + drop-in-place). **AI-suggest in the library deferred** (search shipped).
- **Task 4.4** — ✅ **Done 2026-06-01** — `TemplateBlockMapping.MapUnitsToBlocks` (pure) maps a template version's per-unit sticker groups → one published `Block` per unit (activities referenced in order, `primary` role); `BlockMaterializer.MaterializeAsync` (additive, idempotent by name) inserts only Block rows; `POST /api/album-templates/{id}/derive-blocks` wires it up. Tests: `compat_mapping_roundtrip` (mapper) + `instance_survives` (materialize leaves instance/evidence/instance-stickers untouched). Verified live (7 blocks derived, evidence 5→5 intact). **Deliberate scope:** the proven `InstanceSticker` mint path is left intact (it already snapshots references); wiring the mint *through* blocks waits until a template composes blocks (Phase 5/6) — rewiring it now would risk evidence on live albums for no gain.
...
- **Task 5.1** — ✅ **Done 2026-06-02** — `Topic`/`TopicVersion`/`TopicBlockRelation` (reference) entities + `20260602120000_AddTopics` migration, mirroring block versioning (≤1 draft, published-immutable). `Topic → Block` directly. Endpoints behind `Features:Hierarchy:Topic`. 2 InMemory tests (references-not-copies, same-block-two-topics) + full flow verified live.
- **Task 5.2** — ✅ **Done 2026-06-02** — `TopicBuilderComponent` (composes `CreationShellComponent`): left published-block library, center CDK drag-&-drop block references (reorder/remove), right progression preview. Route `/teacher/topics` + flag-gated „Témakörök" nav. `BlockListItem.latestPublishedVersionId` added so topics reference published blocks only. 30/30 ng-test.
- **Task 5.3** — ⏸️ **Deferred 2026-06-02 (gating condition not met).** The trigger was "*iff GATE-1 retires the week model*", but GATE-1 retired the `Tanulási egység` **level**, not the week/unit grid (`DurationType` + unit index is intact). A `Week`/`WeekNumber`/`CurrentWeek` → `UnitIndex` rename would be pure cosmetic churn across persisted columns on **running albums**, every API DTO/endpoint, and the **whole web app** — high regression risk, no functional gain, and against the plan's own "no premature DB/API rename" guardrail. Stays in `backlog.md` until a real week-model change forces schema churn. (Same risk-management stance as the deferred 4.4 mint-rewire.)
